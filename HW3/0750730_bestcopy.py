...
# Train data to do train_test_split for validation
X_train, X_test, y_train, y_test = train_test_split(x_train, y_all_train,
                                                    test_size=0.25,
                                                    random_state=33)

# RandomForestClassifier
# rfc = RandomForestClassifier(max_depth=13, max_features=37,
...
model = GridSearchCV(rfc, optimization_dict, scoring='accuracy', verbose=1,
                     cv=5, n_jobs=-2)
model.fit(x_train,y_all_train)
print(model.best_score_)
print(model.best_params_)

rfc = RandomForestClassifier(**model.best_params_)
...
gbc = GradientBoostingClassifier(learning_rate=0.21, max_depth=4,
                                 ...
                                 subsample=0.9)
# gbc = GradientBoostingClassifier(learning_rate=0.21, max_depth=4,
#                                  max_features=35, min_samples_leaf=10,
...
model = GridSearchCV(gbc, optimization_dict, scoring='accuracy', verbose=1,
                     cv=5, n_jobs=-2)
model.fit(x_train,y_all_train)
print(model.best_score_)
print(model.best_params_)
...
# xgbc = XGBClassifier(learning_rate=0.32,min_child_weight=0.475,max_depth=4,
#                      gamma=0.48,subsample=0.95,colsample_bytree=0.88,
#                      colsample_bylevel=0.49,n_estimators=124)
...
model = GridSearchCV(xgbc, optimization_dict, scoring='accuracy', verbose=1,
                     cv=5, n_jobs=-2)
model.fit(x_train,y_all_train)
print(model.best_score_)
print(model.best_params_)
